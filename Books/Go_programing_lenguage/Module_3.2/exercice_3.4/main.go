...
func main() {
	...
	http.HandleFunc("/egg-box", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/svg+xml")
		fmt.Fprint(w, generateSVG("g"))
	})

	http.HandleFunc("/moguls", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/svg+xml")
		fmt.Fprint(w, generateSVG("h"))
	})

	http.HandleFunc("/saddle", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/svg+xml")
		fmt.Fprint(w, generateSVG("w"))
	})
	log.Fatal(http.ListenAndServe(":8000", nil))
}

func generateSVG(funcType string) string {
	var svg string
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			ax, ay := corner(i+1, j, funcType)
			bx, by := corner(i, j, funcType)
			cx, cy := corner(i, j+1, funcType)
			dx, dy := corner(i+1, j+1, funcType)
			if math.IsNaN(ax) || math.IsNaN(ay) || math.IsNaN(bx) || math.IsNaN(by) ||
				math.IsNaN(cx) || math.IsNaN(cy) || math.IsNaN(dx) || math.IsNaN(dy) {
				continue
			}

			z := f(xyrange*(float64(i+1)/cells-0.5), xyrange*(float64(j)/cells-0.5))
			color := colorForZ(z)
...
func corner(i, j int, funcType string) (float64, float64) {
